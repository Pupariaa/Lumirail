...
#include <USBCDC.h>
USBCDC PcLink;
HardwareSerial LogUart(0);
#else
HardwareSerial PcLink(0);
#endif
...
static void usbDrain(void) {
  for (int n = 0; n < 64 && PcLink.availableForWrite() > 0 && moduleToUsbCount > 0; n++) {
    PcLink.write(moduleToUsbBuf[moduleToUsbHead]);
    moduleToUsbHead++;
...
void setup() {
...
#endif
  if (lmsSdMount()) {
    if (lmsSdMountedWithFormat())
      usbPrint("SD_OK_FORMAT\n");
    else
      usbPrint("SD_OK\n");
  } else
    usbPrint("SD_FAIL\n");
  ...
  usbPrint(LMS_L1_BOOT_MSG);
  for (int i = 0; i < 512 && moduleToUsbCount > 0; i++)
    usbDrain();
  PcLink.flush();
}
...
void loop() {
  delay(0);
  status_leds_tick(sceneSendState);
  usbDrain();
  pollModuleOneByte();
  ...
  if (sceneState && sceneRecvPhase == RECV_PHASE_LINE && PcLink.available() > 0) {
    char c = (char)PcLink.read();
    if (c == '\n' || c == '\r') {
  ...
  if (sceneState && sceneRecvPhase == RECV_PHASE_BINARY && sceneRecvBlockBufLen < sceneRecvBlockLen && PcLink.available() > 0) {
    sceneRecvBlockBuf[sceneRecvBlockBufLen++] = (uint8_t)PcLink.read();
  ...
  if (!sceneState && !sceneSendState && PcLink.available() > 0) {
    char c = (char)PcLink.read();
    if (c == '\n' || c == '\r') {
      if (lineLen > 0) {
        lineBuf[lineLen] = '\0';
