...
let port: (SerialPort & { readable: ReadableStream<Uint8Array>; writable: WritableStream<Uint8Array> }) | PortLike | null = null
...
async function attachPort(p: PortLike): Promise<void> {
  port = p
...
async function connectAuto(): Promise<void> {
  const serial = getSerial()
  if (!serial) {
    throw new Error('Web Serial is not supported')
  }
        ...
        try {
          ...
          if (vid === DIGIKEY_VID && pid === DIGIKEY_PID) {
            ...
            if (typeof portCandidate.setSignals === 'function') {
              await portCandidate.setSignals({ dataTerminalReady: true, requestToSend: false })
              await new Promise((r) => setTimeout(r, SIGNALS_DELAY_MS))
            }
            p = portCandidate
            break
...
async function disconnect(): Promise<void> {
  ...
  if (port) {
    const p = port as { close?: () => Promise<void> }
...
function isSupported(): boolean {
  ...
  if (typeof window !== 'undefined' && window.electronSerial) return true
  return false
}

type ElectronAdapter = { getPort: () => PortLike | null }

async function connectWithAdapter(adapter: ElectronAdapter): Promise<void> {
  if (state === 'connected') return
  const p = adapter.getPort()
  if (!p) throw new Error('Adapter not connected')
  setState('connecting')
  try {
    await attachPort(p)
  } catch (err) {
